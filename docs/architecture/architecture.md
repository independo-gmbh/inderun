...
The TypeScript Engine core (`packages/inderun-web/src/core/`) implements the Mode 2 orchestrator against these contracts: `IndeRun.stream()` selects a route the same way `run()` does, restricted to providers that declare `supports.streaming` and implement the optional `ProviderAdapter.stream()` method, and returns a `StreamRunHandle` plus an `AsyncIterable<StreamEvent>` and a `cancel()` function. No shipped provider (OpenAI, ONNX Runtime, web system-model) implements `stream()` yet — the orchestrator is proven against a deterministic test-only provider — because real network streaming needs a host-services capability (chunked/SSE HTTP) that does not exist yet; see `docs/architecture/providers.md` for current per-provider status.
...
Cancellation always forecloses fallback: once a signal is observed as aborted, the engine synthesizes the `cancelled` terminal outcome and stops, regardless of whether any content has been delivered yet. Independent of cancellation, streaming fallback itself is narrower than Mode 1's: a provider failure is only fallback-eligible *before* the first content event (`content_delta`/`content_snapshot`) has been admitted for the run. Once one has been delivered, the run is committed to that provider — a later provider failure becomes a terminal `error` outcome, never a silent provider swap, since splicing partial text from two different providers would be both undetectable to the caller and a leak of provider-specific mechanics.
